 ...
 * 而小于等于n的正整数有n个，也就是 p^k个。 减去不互质的 p^(k-1)个 得到 φ(n)=p^k-p^(k-1) = p^k*(1-1/p) 。
 * 
 ...
 * 
 * φ(n)=φ(p*q)=φ(p)φ(q)=φ(p1^k1)φ(p2^k2)
 * ------------------------------------------------------
 * 
 * 证明:
 * 
 ...
 * 任意一个大于一的正整。
 * 
 * φ(n)=n(1-1/p1)(1-1/p2)...(1-1/pr) 其中 p1、p2、p3...pr是n的质因数。
 * ------------------------------------------------------
 * 证明:
 * 
 * 任意一个大于一的正整数都能用一连串的质因数相乘表示 [ n=(p1^k1)*(p2^k2)*(p3^k3)...(pr^kr) ] 。
 * 
 * 例如: 100 = 2^2 * 5^2 , 66 = 2^1 * 3^1 * 11^1 等等可以通过前面的 decompositionPrimeFactor 函数得到任意数被分解后的质因数。
 * 
 * φ(n)=φ((p1^k1)*(p2^k2)*(p3^k3)...(pr^kr))=φ(p1^k1)φ(p2^k2)φ(p3^k3)...φ(pr^kr)  // 根据2得到
 * 
 * =(p1^k1)*(p2^k2)*(p3^k3)...(pr^kr)*(1-1/p1)(1-1/p2)...(1-1/pr)  // 根据1得到
 * 
 * =n(1-1/p1)(1-1/p2)...(1-1/pr) <=欧拉函数
 * 
 */
